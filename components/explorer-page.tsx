...
import { AnalyticsAnchor } from '@/components/analytics-controls';
    ...
    <main className="ts-orb min-h-screen">
      ...
      <section className="ts-orb-shell ts-orb-hero">
        <div>
          ...
          <h1 className="ts-orb-hero-title">
            What apartments in your corridor are actually worth.
          </h1>
          ...
          <div className="ts-orb-hero-actions">
            <AnalyticsAnchor
              ...
              eventParams={{
                button_id: 'explorer_explore_market',
                destination: '/buyer',
              }}
            >
          ...
          <p className="ts-orb-note">
            No sign-up, no questionnaire, nothing to fill in.
          </p>
        </div>
        <div className="ts-orb-trust-card">
          ...
          <div className="ts-orb-evidence-strip">
            ...
            <div>
              <strong>{formatInr(medianPricePerSqFt)}</strong>
      ...
      <section className="ts-orb-section">
        <div className="ts-orb-shell ts-orb-section-head">
          <div>
            <p className="ts-orb-eyebrow">WHY THIS EXISTS</p>
            <h2 className="ts-orb-section-title">
              The missing record for Bengaluru apartments.
            </h2>
          </div>
          <div className="space-y-5 text-[15px] leading-7 text-muted-foreground">
            <p>
              Real-estate information in Bengaluru is fragmented, delayed,
              anecdotal, and usually held by someone who earns from the
              transaction.
            </p>
            <p>
              Owners know a ballpark. Buyers know an asking price. Neither can
              easily check either.
            </p>
            <p>
        ...
        <div className="ts-orb-shell text-center">
          ...
          <div className="ts-orb-grid mt-8 text-left">
            <Card className="bg-background text-foreground">
              <CardHeader>
                <CardTitle>By micro-market</CardTitle>
              </CardHeader>
            ...
            <Card className="bg-background text-foreground">
              <CardHeader>
                <CardTitle>By society name</CardTitle>
              </CardHeader>
              <CardContent className="text-sm leading-7 text-muted-foreground">
                Search all supported gated societies.
              </CardContent>
            </Card>
            <Card className="bg-background text-foreground">
              <CardHeader>
                <CardTitle>By budget band</CardTitle>
              </CardHeader>
              <CardContent className="text-sm leading-7 text-muted-foreground">
                Use the V1 budget and BHK filters.
              </CardContent>
            </Card>
          </div>
          ...
          <AnalyticsAnchor
            href="/buyer"
            className="ts-orb-button mt-7 bg-white"
            eventName="primary_cta_click"
            eventParams={{
              button_id: 'explorer_browse_societies',
              destination: '/buyer',
            }}
          >
            BROWSE SOCIETIES
        ...
        <div className="ts-orb-shell">
          <div className="ts-orb-section-head">
            <div>
              ...
              <h2 className="ts-orb-section-title mt-5">
                Our rules, published
              </h2>
            </div>
            <p className="ts-orb-section-copy">
              We&apos;d rather show a wide range than a confident wrong number.
            </p>
          </div>
          <div className="ts-orb-grid">
            {rules.map((rule, index) => (
              <div key={rule} className="ts-orb-choice">
                <span className="font-mono text-[10px]">0{index + 1}</span>
                <h3>{rule}</h3>
                <CheckCircle2 className="mt-auto size-4 text-accent-foreground" />
              </div>
            ))}
          </div>
        </div>
