...
export async function GET(request: Request) {
  ...
  try {
    const sql = getSql();
    const [summaryRows, deliveries, bugReports] = await Promise.all([
      sql`
        SELECT
          ...
          (SELECT COUNT(*)::integer FROM bug_reports WHERE status = 'open') AS open_bug_reports
      `,
      sql`
        SELECT
          deliveries.id,
    ...
    return NextResponse.json(
      { summary: summaryRows[0], deliveries, bugReports },
