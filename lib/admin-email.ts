...
export async function sendAdminOtp(otp: string) {
  ...
  const response = await fetch(
    `https://api.agentmail.to/v0/inboxes/${encodeURIComponent(inboxId)}/messages/send`,
    {
      ...
      body: JSON.stringify({
        to: [ADMIN_EMAIL],
        subject: `${otp} is your FlatData admin code`,
        text: `Your FlatData admin verification code is ${otp}. It expires in 10 minutes. If you did not request this code, you can ignore this email.`,
        html: `
      <div style="font-family:Arial,sans-serif;background:#f6f1e8;padding:32px;color:#221b13">
        <div style="max-width:520px;margin:0 auto;background:#ffffff;border:1px solid #d7cdbc;padding:32px">
          <p style="margin:0 0 24px"><span style="font-family:Georgia,serif;font-size:28px;letter-spacing:-1px">Flat</span><span style="font-family:'Courier New',monospace;font-size:18px;font-weight:700;letter-spacing:-1px">Data</span><span style="font-size:11px;letter-spacing:1.5px;margin-left:8px">ADMIN</span></p>
