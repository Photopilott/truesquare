...
export async function sendUserOtp(email: string, otp: string) {
  ...
  const response = await fetch(
    `https://api.agentmail.to/v0/inboxes/${encodeURIComponent(inboxId)}/messages/send`,
    {
      ...
      body: JSON.stringify({
        to: [email],
        subject: `${otp} is your TrueSquare sign-in code`,
        text: `Your TrueSquare verification code is ${otp}. It expires in 10 minutes. If you did not request this code, you can ignore this email.`,
        html: `
          <div style="font-family:Arial,sans-serif;background:#f6f1e8;padding:32px;color:#221b13">
            <div style="max-width:520px;margin:0 auto;background:#ffffff;border:1px solid #d7cdbc;padding:32px">
              <p style="font-size:12px;letter-spacing:1.5px;margin:0 0 24px">TRUESQUARE</p>
              ...
              <p style="font-size:14px;line-height:1.6;color:#6b6258">This code expires in 10 minutes and can be used once. TrueSquare will never ask you to share this code.</p>
            </div>
          </div>
        `,
      }),
